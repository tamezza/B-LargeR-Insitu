...
// Define mass cuts with 25 GeV bins
const int nMassCuts = 6;
const char* massCutLabels[nMassCuts] = {
    "50-100", "100-150",
    "150-200", "200-250",
    "250-300", "300-350"
};
const double massCutLow[nMassCuts] = {
    50, 100,
    150, 200,
    250, 300
};
const double massCutHigh[nMassCuts] = {
    100, 150,
    200, 250,
    300, 350
};
...
void CreateMassBinnedPlotsDirectories(const char* wp_label) {
    // Base directory for the specific working point
    TString baseDir = Form("../Plots/MJB/2DplotsRdbpTbin/%s/MassBinpT", wp_label);
    gSystem->Exec(Form("mkdir -p %s", baseDir.Data()));
}

// Main processing function for a single working point
void ProcessMassBinnedPlots(const char* wp_label) {
    std::cout << "\n--- Processing Mass Binned Plots for Working Point: " << wp_label << " ---" << std::endl;

    // Create directories for the current working point
    CreateMassBinnedPlotsDirectories(wp_label);
    ...
    std::string dataPath = basePath + "NtupleSlim/MJB_" + wp_label + "wp_slim/data_" + wp_label + "wp_slim.root";
    std::string zbbPath = basePath + "NtupleSlim/MJB_" + wp_label + "wp_slim/ZbbJets_" + wp_label + "wp_slim.root";
    std::string mjPath = basePath + "NtupleSlim/MJB_" + wp_label + "wp_slim/Multijets_" + wp_label + "wp_slim.root";
    ...
    for (int m = 0; m < nMassCuts; m++) {
        ...
        for (int bin = 0; bin < nPtBins; bin++) {
            // MC signal fit
            TF1 *gausFit_mc = new TF1(Form("gausFit_mc_masscut%s_bin%d_%s", massCutLabels[m], bin, wp_label), "gaus", 0.875, 1.125);
            ...
            // Data fit
            TF1 *gausFit_data = new TF1(Form("gausFit_data_masscut%s_bin%d_%s", massCutLabels[m], bin, wp_label), "gaus", 0.875, 1.125);
            ...
            // Background fit
            TF1 *gausFit_bkg = new TF1(Form("gausFit_bkg_masscut%s_bin%d_%s", massCutLabels[m], bin, wp_label), "gaus", 0.875, 1.125);
            ...
            // Combined MC fit
            TF1 *gausFit_combined = new TF1(Form("gausFit_combined_masscut%s_bin%d_%s", massCutLabels[m], bin, wp_label), "gaus", 0.875, 1.125);
    ...
    // Create a mtest plot comparing all mass cuts in one figure (for data/combined MC ratio)
    ...
    all_ratio_canvas->SaveAs(Form("../Plots/MJB/2DplotsRdbpTbin/%s/MassBinpT/R_DB_all_ratio_mtest.pdf", wp_label));
    ...
    data_means_canvas->SaveAs(Form("../Plots/MJB/2DplotsRdbpTbin/%s/MassBinpT/Data_R_MJB_mean_values.pdf", wp_label));
    ...
    mc_means_canvas->SaveAs(Form("../Plots/MJB/2DplotsRdbpTbin/%s/MassBinpT/MC_R_MJB_mean_values.pdf", wp_label));
    ...
    bkg_means_canvas->SaveAs(Form("../Plots/MJB/2DplotsRdbpTbin/%s/MassBinpT/Bkg_R_MJB_mean_values.pdf", wp_label));
    ...
    combined_means_canvas->SaveAs(Form("../Plots/MJB/2DplotsRdbpTbin/%s/MassBinpT/Combined_MC_R_MJB_mean_values.pdf", wp_label));
    delete combined_means_canvas;
    delete combined_means_map;


    // Create a mtest table file
    ofstream mtestTable(Form("../Plots/MJB/2DplotsRdbpTbin/%s/MassBinpT/R_DB_mtest_table.txt", wp_label));
    if (mtestTable.is_open()) {
        mtestTable << "R_DB MTEST TABLE FOR ALL MASS CUTS AND PT BINS (WP: " << wp_label << ")" << std::endl;
        mtestTable << "================================================" << std::endl << std::endl;
        
        for (int m = 0; m < nMassCuts; m++) {
            mtestTable << "MASS CUT: " << massCutLow[m] << " < m < " << massCutHigh[m] << " GeV" << std::endl;
            mtestTable << "------------------------------------------------" << std::endl;
            
            // Table header
            mtestTable << std::left << std::setw(15) << "pT Bin" 
            ...
            for (int bin = 0; bin < nPtBins; bin++) {
                ...
                mtestTable << std::left << std::setw(15) << ptBinLabels[bin]
        ...
        mtestTable.close();
        std::cout << "Mtest table saved to ../Plots/MJB/2DplotsRdbpTbin/" << wp_label << "/MassBinpT/R_DB_mtest_table.txt" << std::endl;
    } else {
    ...
    std::cout << "Finished processing Mass Binned Plots for Working Point: " << wp_label << std::endl;
}

// Main function to run analysis for all specified working points
void CreateMassBinnedPlots() {
    ...
    for (const char* wp : workingPoints) {
        ProcessMassBinnedPlots(wp);
    }
    std::cout << "\n--- All working points for Mass Binned Plots processed ---" << std::endl;
}