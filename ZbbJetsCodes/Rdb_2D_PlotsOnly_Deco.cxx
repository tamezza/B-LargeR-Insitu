...
void Create2DPlotDirectories(const char* wp_label) {
    ...
    TString baseDir = Form("../Plots/MJB/2DplotsRdbpTbin/%s", wp_label);
...
void Process2DPlotsOnly(const char* wp_label) {
    ...
    if (palette) {
        ...
        palette_text.DrawLatex(0.97, 0.5, "#LT R_{MJB}^{Data} #GT / #LT R_{MJB}^{MC} #GT"); // Position to the right of palette
    }

    // Draw ATLAS label outside the plot (in the top margin)
    DrawAtlasLabel();

    all_ratio_canvas->SaveAs(Form("../Plots/MJB/2DplotsRdbpTbin/%s/Summary/R_DB_all_ratio_summary.pdf", wp_label));
    ...
    //------------------------------------------------------------
    TCanvas* data_means_canvas = new TCanvas(Form("data_means_canvas_%s", wp_label), Form("Data R_{MJB} Mean Values (WP: %s)", wp_label), 800, 600);
    ...
    // Create a 2D plot
    TH2F* data_means_map = new TH2F(Form("data_means_map_%s", wp_label), Form("Data R_{MJB} Mean Values (WP: %s);Mass Cut;p_{T} Bin", wp_label), nMassCuts, -0.5, nMassCuts-0.5, nPtBins, -0.5, nPtBins-0.5);
    ...
    if (data_palette) {
        ...
        data_palette_text.DrawLatex(0.97, 0.5, "#LT R_{MJB}^{Data} #GT");
    }

    // Draw ATLAS label outside the plot
    DrawAtlasLabel();

    data_means_canvas->SaveAs(Form("../Plots/MJB/2DplotsRdbpTbin/%s/Summary/Data_R_MJB_Deco_mean_values.pdf", wp_label));
    ...
    //------------------------------------------------------------
    TCanvas* mc_means_canvas = new TCanvas(Form("mc_means_canvas_%s", wp_label), Form("MC R_{MJB} Mean Values (WP: %s)", wp_label), 800, 600);
    ...
    // Create a 2D plot
    TH2F* mc_means_map = new TH2F(Form("mc_means_map_%s", wp_label), Form("MC R_{MJB} Mean Values (WP: %s);Mass Cut;p_{T} Bin", wp_label), nMassCuts, -0.5, nMassCuts-0.5, nPtBins, -0.5, nPtBins-0.5);
    ...
    if (mc_palette) {
        ...
        mc_palette_text.DrawLatex(0.97, 0.5, "#LT R_{MJB}^{Signal MC} #GT");
    }

    // Draw ATLAS label outside the plot
    DrawAtlasLabel();

    mc_means_canvas->SaveAs(Form("../Plots/MJB/2DplotsRdbpTbin/%s/Summary/MC_R_MJB_Deco_mean_values.pdf", wp_label));
    ...
    //------------------------------------------------------------
    TCanvas* bkg_means_canvas = new TCanvas(Form("bkg_means_canvas_%s", wp_label), Form("Background R_{MJB} Mean Values (WP: %s)", wp_label), 800, 600);
    ...
    // Create a 2D plot
    TH2F* bkg_means_map = new TH2F(Form("bkg_means_map_%s", wp_label), Form("Background R_{MJB} Mean Values (WP: %s);Mass Cut;p_{T} Bin", wp_label), nMassCuts, -0.5, nMassCuts-0.5, nPtBins, -0.5, nPtBins-0.5);
    ...
    if (bkg_palette) {
        ...
        bkg_palette_text.DrawLatex(0.97, 0.5, "#LT R_{MJB}^{Multijets} #GT");
    }

    // Draw ATLAS label outside the plot
    DrawAtlasLabel();

    bkg_means_canvas->SaveAs(Form("../Plots/MJB/2DplotsRdbpTbin/%s/Summary/Bkg_R_MJB_Deco_mean_values.pdf", wp_label));
    ...
    //------------------------------------------------------------
    TCanvas* combined_means_canvas = new TCanvas(Form("combined_means_canvas_%s", wp_label), Form("Combined MC R_{MJB} Mean Values (WP: %s)", wp_label), 800, 600);
    ...
    // Create a 2D plot
    TH2F* combined_means_map = new TH2F(Form("combined_means_map_%s", wp_label), Form("Combined MC R_{MJB} Mean Values (WP: %s);Mass Cut;p_{T} Bin", wp_label), nMassCuts, -0.5, nMassCuts-0.5, nPtBins, -0.5, nPtBins-0.5);
    ...
    if (combined_palette) {
        ...
        combined_palette_text.DrawLatex(0.97, 0.5, "#LT R_{MJB}^{Combined MC} #GT");
    }

    // Draw ATLAS label outside the plot
    DrawAtlasLabel();

    combined_means_canvas->SaveAs(Form("../Plots/MJB/2DplotsRdbpTbin/%s/Summary/Combined_MC_R_MJB_Deco_mean_values.pdf", wp_label));
    delete combined_means_canvas;
    delete combined_means_map;


    // Create a summary table file
    ofstream summaryTable(Form("../Plots/MJB/2DplotsRdbpTbin/%s/Summary/R_DB_summary_table.txt", wp_label));
    if (summaryTable.is_open()) {
        ...
        summaryTable.close();
        std::cout << "Summary table saved to ../Plots/MJB/2DplotsRdbpTbin/" << wp_label << "/Summary/R_DB_summary_table.txt" << std::endl;
    } else {
