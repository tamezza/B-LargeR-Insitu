 ...
 */
 ...
 struct WorkingPoint {
    ...
    WorkingPoint(const std::string& wp) : name(wp) {
        ...
        /*inputFiles = {
            eosPath + "NtupleSlim/MJB_Deco_0p" + wp + "wp_slim/data_deco_0p" + wp + "wp_slim.root",          
            eosPath + "NtupleSlim/MJB_Deco_0p" + wp + "wp_slim/Multijets_deco_0p" + wp + "wp_slim.root",      
            eosPath + "NtupleSlim/MJB_Deco_0p" + wp + "wp_slim/ZbbJets_deco_0p" + wp + "wp_slim.root"         
        };
        
        outputDir = "../Plots/MJB_Deco/" + wp + "/MassRdb/";*/
        ...
        inputFiles = {
            eosPath + "NtupleSlim/MJB_Deco_" + fileWP + "wp_slim/data_deco_" + fileWP + "wp_slim.root",          
            eosPath + "NtupleSlim/MJB_Deco_" + fileWP + "wp_slim/Multijets_deco_" + fileWP + "wp_slim.root",      
            eosPath + "NtupleSlim/MJB_Deco_" + fileWP + "wp_slim/ZbbJets_deco_" + fileWP + "wp_slim.root"         
        };
        //outputDir = "../Plots/MJB_Deco/" + fileWP + "/MassResponse/";
        outputDir = "../Plots/MJB_Deco/MassResponse/" + fileWP + "/";
    }
 ...
 /**
  * Main class for histogram management and analysis bJR10v00
  */
 class HistogramManager {
 private:
     std::vector<TH1F*> h_ljet_mass;
     std::vector<TH1F*> h_ljet_mass_50_350;
     std::vector<TH1F*> h_ljet_mass_50_150;
     std::vector<TH1F*> h_ljet_mass_50_150_2;
     ...
     void InitializeHistogramGroup(std::vector<TH1F*>& histograms, const std::string& variable, 
         ...
         std::vector<std::string> names = { "data", "MultiJets", "Zbbjets" };
 ...
 public:
     ...
     ~HistogramManager() {
         ...
         for (auto hist : h_ljet_mass) delete hist;
         for (auto hist : h_ljet_mass_50_350) delete hist;
         for (auto hist : h_ljet_mass_50_150) delete hist;
         for (auto hist : h_ljet_mass_50_150_2) delete hist;
     ...
     void InitializeHistograms() {
         InitializeHistogramGroup(h_ljet_mass, "ljet_bJR10v00_mass", "Large-R Jet Mass;M^{J} [GeV];Events", 50, 50, 200);
         InitializeHistogramGroup(h_ljet_mass_50_350, "ljet_bJR10v00_mass_50_350", "Large-R Jet Mass;M^{J} [GeV];Events", 50, 50, 350);
         InitializeHistogramGroup(h_ljet_mass_50_150, "ljet_bJR10v00_mass_50_150", "Large-R Jet Mass;M^{J} [GeV];Events", 20, 50, 150);
         InitializeHistogramGroup(h_ljet_mass_50_150_2, "ljet_bJR10v00_mass_50_150_2", "Large-R Jet Mass;M^{J} [GeV];Events", 50, 50, 150);
         ...
         SetHistogramColors(h_ljet_mass);
         SetHistogramColors(h_ljet_mass_50_350);
         SetHistogramColors(h_ljet_mass_50_150);
         SetHistogramColors(h_ljet_mass_50_150_2);
      ...
      * @param ljet_bJR10v00_mass Output vector for jet mass
      ...
      */
     void SetBranchAddresses(TTree* tree, std::vector<float>*& ljet_bJR10v00_mass, double& weight, 
                            double& Asym, double& R_DB, 
                            std::vector<int>*& ljet_flavour_label, 
                            std::vector<int>*& ljet_truth_label) {
         // Set addresses for basic variables
         tree->SetBranchAddress("ljet_bJR10v00_mass", &ljet_bJR10v00_mass);
     ...
     void FillHistograms(TTree* tree, std::vector<TH1F*>& histograms, bool isData) {
         ...
         SetBranchAddresses(tree, ljet_bJR10v00_mass, weight, Asym, R_DB, 
         ...
         for (Long64_t i = 0; i < nentries; ++i) {
             ...
             histograms[0]->Fill((ljet_bJR10v00_mass->at(0))/1000.0, eventWeight);
             histograms[1]->Fill((ljet_bJR10v00_mass->at(0))/1000.0, eventWeight);
             histograms[2]->Fill((ljet_bJR10v00_mass->at(0))/1000.0, eventWeight);
             histograms[3]->Fill((ljet_bJR10v00_mass->at(0))/1000.0, eventWeight);
             ...
             if (!isData && ljet_flavour_label && ljet_truth_label) {
                 ...
                 if (jet_bin < h_R_DB_bins.size()) {
                     ...
                     h_mass_bins[jet_bin]->Fill(ljet_bJR10v00_mass->at(0), weight);
                 }
             }
         }
     ...
     void ProcessFiles(const std::vector<std::string>& files) {
         ...
         for (size_t i = 0; i < trees.size(); ++i) {
             ...
             std::vector<TH1F*> histograms = {
                 h_ljet_mass[i], h_ljet_mass_50_350[i], h_ljet_mass_50_150[i], 
                 h_ljet_mass_50_150_2[i], h_Asym[i], h_Asym_2[i], 
     ...
     void CalculateAndApplyScaleFactor() {
         ...
         int binLow1 = h_ljet_mass_50_150[1]->FindBin(50);
         int binHigh1 = h_ljet_mass_50_150[1]->FindBin(65);
         int binLow2 = h_ljet_mass_50_150[1]->FindBin(110);
         int binHigh2 = h_ljet_mass_50_150[1]->FindBin(150);
         ...
         for (int bin = binLow1; bin <= binHigh1; ++bin) {
             double bin_center = h_ljet_mass_50_150[1]->GetBinCenter(bin);
             double data_content = h_ljet_mass_50_150[0]->GetBinContent(bin);
             double mc_content = h_ljet_mass_50_150[1]->GetBinContent(bin);
             ...
             if (mc_content > 0) {
                 ...
                 double data_error = h_ljet_mass_50_150[0]->GetBinError(bin);
                 double mc_error = h_ljet_mass_50_150[1]->GetBinError(bin);
         ...
         std::vector<TH1F*> multijetHistograms = {
             h_ljet_mass[1], h_ljet_mass_50_350[1], h_ljet_mass_50_150[1],
             h_ljet_mass_50_150_2[1]
         };
     ...
     void SaveHistograms(bool createRatioPlots = true) {
         ...
         std::vector<std::string> canvasNames = {
             "Large_R_Jet_bJR10v00_mass", "Large_R_Jet_bJR10v00_mass_50_350", "Large_R_Jet_bJR10v00_mass_50_150",
             "Large_R_Jet_bJR10v00_mass_50_150_2", "Asymmetry", "Asymmetry_2",
             "R_DB_1", "R_DB_2", "R_DB_3", "R_DB"
         };
 
         // Group histograms for saving
         std::vector<std::vector<TH1F*>> histogramGroups = {
             h_ljet_mass, h_ljet_mass_50_350, h_ljet_mass_50_150, h_ljet_mass_50_150_2,
 ...
 private:
     ...
     void SaveHistogramGroup(std::vector<TH1F*>& histograms, const std::string& name, 
         ...
         if (createRatioPlot) {
             ...
             legend->AddEntry(histograms[2], "Z (#rightarrow bb) + jets", "f");
             legend->AddEntry(histograms[1], "Multijets", "f");
            ...
            /*
            if (name.find("Large_R_Jet_bJR10v00_mass") != std::string::npos) {
     ...
     void DrawSplitHistograms(const std::vector<std::unique_ptr<TH1F>>& histograms, TCanvas* canvas) {
         ...
         for (size_t i = 0; i < histograms.size(); ++i) {
             ...
             legend->AddEntry(histograms[i].get(), binNames[i].c_str(), "l");
         }
  ...
  */
 void Mass_Response_Deco() {
     gROOT->LoadMacro("AtlasStyle.C");
     SetAtlasStyle();
 
     // Define working points to process
     //std::vector<std::string> workingPoints = {"50", "55", "60", "65"};
     //std::vector<std::string> workingPoints = {"25", "30", "37", "46", "58", "74", "94", "125", "155"};//{"25"};
     ...
     for (const auto& wp : workingPoints) {
         ...
         HistogramManager histManager(wp, workingPoint.outputDir);
         histManager.ProcessFiles(workingPoint.inputFiles);
         histManager.SaveHistograms();
