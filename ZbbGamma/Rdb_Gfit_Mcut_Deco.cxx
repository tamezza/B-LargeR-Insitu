...
void DrawAtlasLabel(const char* wp_label) {
    ...
    latex.DrawLatex(0.18, 0.87, "ATLAS"); // Adjusted position for consistency

    // Draw "Internal" label
    latex.SetTextFont(42);
    latex.DrawLatex(0.3, 0.87, "Internal"); // Adjusted position for consistency

    // Draw the energy and luminosity text
    latex.DrawLatex(0.18, 0.82, "#sqrt{s} = 13 TeV, 140 fb^{-1}"); // Adjusted position for consistency
    
    // Draw working point label
    latex.SetTextSize(0.03);
    latex.DrawLatex(0.18, 0.78, Form("Working Point: %s", wp_label)); // Adjusted position
}
...
void CreateDirectories(const char* wp_label) {
    ...
    TString baseDirCmd = Form("mkdir -p ../Plots/DB_Deco/FitRdbMcut/%s", wp_label);
    ...
    //const int nMassCuts = 3;
    //const char* massCutLabels[nMassCuts] = {"50-80GeV", "80-110GeV", "#geq110 GeV"};

    // Create a directory for each mass cut within the working point directory
    for (int i = 0; i < nMassCuts; i++) {
        TString dirCmd = Form("mkdir -p ../Plots/DB_Deco/FitRdbMcut/%s/%s", wp_label, fileSystemMassCutLabels[i]);
...
void RunRdbAnalysis(const char* wp_label) {
    ...
    // Define the mass bins we want to analyze
    /*const int nMassCuts = 3;
    const char* massCutLabels[nMassCuts] = {"50-80GeV", "80-110GeV", "#geq110 GeV"};
    const double massCutLow[nMassCuts] = {50, 80, 110};  // Lower bound for each bin
    const double massCutHigh[nMassCuts] = {80, 110, -1}; // Upper bound for each bin (-1 means no upper bound)*/
    ...
    //-------------------------------------------
    TString mcFileName = Form("../../NtupleSlim/DB_Deco_0p%swp_slim/Zbby_deco_0p%swp_slim.root", wp_label, wp_label);
    TString dataFileName = Form("../../NtupleSlim/DB_Deco_0p%swp_slim/data_deco_0p%swp_slim.root", wp_label, wp_label);
    TString mulijetsFileName = Form("../../NtupleSlim/DB_Deco_0p%swp_slim/gamma_jets_deco_0p%swp_slim.root", wp_label, wp_label);
    ...
    TH1F* h_mc_ljet_bJR10v00_mass1[nMassCuts];
    TH1F* h_data_ljet_bJR10v00_mass1[nMassCuts];
    TH1F* h_multijets_ljet_bJR10v00_mass1[nMassCuts];
    ...
    for (int m = 0; m < nMassCuts; m++) {
        ...
        h_mc_ljet_bJR10v00_mass1[m] = new TH1F(Form("h_mc_ljet_bJR10v00_mass1_%s_%s", wp_label, massCutLabels[m]), "ljet_bJR10v00_mass", 20, 50, 150);
        h_data_ljet_bJR10v00_mass1[m] = new TH1F(Form("h_data_m1_%s_%s", wp_label, massCutLabels[m]), "ljet_bJR10v00_mass", 20, 50, 150);
        h_multijets_ljet_bJR10v00_mass1[m] = new TH1F(Form("h_multijets_ljet_bJR10v00_mass1_%s_%s", wp_label, massCutLabels[m]), "ljet_bJR10v00_mass", 20, 50, 150);
        h_mc_ljet_bJR10v00_mass1[m]->Sumw2(); // Enable error calculation
        h_data_ljet_bJR10v00_mass1[m]->Sumw2();
        h_multijets_ljet_bJR10v00_mass1[m]->Sumw2();

        // R_DB histograms (main analysis focus)
        h_mc_R_DB[m] = new TH1F(Form("h_mc_R_DB_%s_%s", wp_label, massCutLabels[m]), "R_DB", 50, 0, 2);
        h_data_R_DB[m] = new TH1F(Form("h_data_R_DB_%s_%s", wp_label, massCutLabels[m]), "R_DB", 50, 0, 2);
        h_multijets_ljet_R_DB[m] = new TH1F(Form("h_multijets_ljet_R_DB_%s_%s", wp_label, massCutLabels[m]), "R_DB", 50, 0, 2);
        ...
        h_data_ljet_bJR10v00_mass1[m]->SetFillColor(kBlack);
        h_multijets_ljet_bJR10v00_mass1[m]->SetFillColor(kBlue);
        h_mc_ljet_bJR10v00_mass1[m]->SetFillColor(kRed);
        ...
        h_data_ljet_bJR10v00_mass1[m]->SetLineColor(kBlack);
        h_multijets_ljet_bJR10v00_mass1[m]->SetLineColor(kBlue);
        h_mc_ljet_bJR10v00_mass1[m]->SetLineColor(kRed);
    ...
    //-------------------------------------------
    std::vector<float>* ljet_bJR10v00_mass = nullptr;
    double weight, weight_multijets;
    double R_DB;

    // Set branch addresses for all trees
    tree_mc->SetBranchAddress("ljet_bJR10v00_mass", &ljet_bJR10v00_mass);
    tree_data->SetBranchAddress("ljet_bJR10v00_mass", &ljet_bJR10v00_mass);
    tree_mulijets->SetBranchAddress("ljet_bJR10v00_mass", &ljet_bJR10v00_mass);
    ...
    for (Long64_t i = 0; i < nentries; i++) {
        tree_mc->GetEntry(i);
        // Ensure ljet_bJR10v00_mass is not empty before accessing its first element
        if (ljet_bJR10v00_mass->empty()) continue;
        double jetMass = (*ljet_bJR10v00_mass)[0]/1000.0;  // Convert MeV to GeV
        ...
        for (int m = 0; m < nMassCuts; m++) {
            ...
            bool passesCut = false; // Initialize to false

            // Explicitly handle mass cut logic for each bin
            if (massCutHigh[m] == -1) { // This is the last bin: >= lower bound, no upper limit
                if (jetMass >= massCutLow[m]) {
                    passesCut = true;
                }
            } else { // This is a finite bin: >= lower bound and < upper bound
                if (jetMass >= massCutLow[m] && jetMass < massCutHigh[m]) {
                    passesCut = true;
                }
            }
            ...
            if (passesCut) {
                h_mc_ljet_bJR10v00_mass1[m]->Fill(jetMass, weight);
    ...
    for (Long64_t i = 0; i < nentries; i++) {
        ...
        for (int m = 0; m < nMassCuts; m++) {
            ...
            // Fill histograms if passes cut
            if (passesCut) {
                h_data_ljet_bJR10v00_mass1[m]->Fill(jetMass);
    ...
    for (Long64_t i = 0; i < nentries; i++) {
        ...
        for (int m = 0; m < nMassCuts; m++) {
            ...
            // Fill histograms if passes cut
            if (passesCut) {
                h_multijets_ljet_bJR10v00_mass1[m]->Fill(jetMass, weight_multijets);
    ...
    for (int m = 0; m < nMassCuts; m++) {
        cout << "==========================================" << endl;
        cout << "Processing Mass Bin: " << massCutLabels[m] << " for Working Point: " << wp_label << endl;
        cout << "==========================================" << endl;
        ...
        int binLow1 = h_multijets_ljet_bJR10v00_mass1[m]->FindBin(50);
        int binHigh1 = h_multijets_ljet_bJR10v00_mass1[m]->FindBin(65);
        int binLow2 = h_multijets_ljet_bJR10v00_mass1[m]->FindBin(110);
        int binHigh2 = h_multijets_ljet_bJR10v00_mass1[m]->FindBin(150);
        ...
        for (int bin = binLow1; bin <= binHigh1; ++bin) {
            double bin_center = h_multijets_ljet_bJR10v00_mass1[m]->GetBinCenter(bin);
            double data_content = h_data_ljet_bJR10v00_mass1[m]->GetBinContent(bin);
            double mc_content = h_multijets_ljet_bJR10v00_mass1[m]->GetBinContent(bin);
            ...
            if (mc_content > 0) {
                ...
                double data_error = h_data_ljet_bJR10v00_mass1[m]->GetBinError(bin);
                double mc_error = h_multijets_ljet_bJR10v00_mass1[m]->GetBinError(bin);
        ...
        printf("Mass Bin: %s, Working Point: %s, Linear fit results: SF(m) = %.4f (+- %.4f) + %.8f (+- %.8f) * m\n", 
        ...
        if (m == 0) {
            ...
            latex.DrawLatex(0.2, 0.75, Form("Mass Bin: %d < m < %d GeV", (int)massCutLow[m], (int)massCutHigh[m]));
        }
        
        fit_canvas->SaveAs(Form("../Plots/DB_Deco/FitRdbMcut/%s/%s/scale_factor_fit.pdf", wp_label, fileSystemMassCutLabels[m]));
        
        //-------------------------------------------
        // Apply scale factor to background histogram
        //-------------------------------------------
        // Make a copy first to preserve the original
        TH1F* h_multijets_ljet_bJR10v00_mass1_scaled = (TH1F*)h_multijets_ljet_bJR10v00_mass1[m]->Clone(Form("h_multijets_ljet_bJR10v00_mass1_scaled_%s_%s", wp_label, massCutLabels[m]));
        
        // Apply mass-dependent scale factor bin by bin
        for (int bin = 1; bin <= h_multijets_ljet_bJR10v00_mass1_scaled->GetNbinsX(); ++bin) {
            double bin_center = h_multijets_ljet_bJR10v00_mass1_scaled->GetBinCenter(bin);
            double content = h_multijets_ljet_bJR10v00_mass1_scaled->GetBinContent(bin);
            double error = h_multijets_ljet_bJR10v00_mass1_scaled->GetBinError(bin);
            
            // Calculate bin-by-bin scale factor
            double scale_factor = p0 + p1 * bin_center;
            
            // Apply the scale factor
            h_multijets_ljet_bJR10v00_mass1_scaled->SetBinContent(bin, content * scale_factor);
            h_multijets_ljet_bJR10v00_mass1_scaled->SetBinError(bin, error * scale_factor);
        }
        ...
        for (int bin = 1; bin <= h_multijets_ljet_bJR10v00_mass1[m]->GetNbinsX(); ++bin) {
            double bin_center = h_multijets_ljet_bJR10v00_mass1[m]->GetBinCenter(bin);
            double scale_factor = p0 + p1 * bin_center;
        ...
        printf("Mass Bin: %s, Working Point: %s, Average Scale Factor: %.4f\n", massCutLabels[m], wp_label, avgScaleFactor);
        ...
        double estimatedBackground = h_multijets_ljet_bJR10v00_mass1_scaled->Integral(binHigh1, binLow2);
        
        // Calculate signal events
        double dataSignal = h_data_ljet_bJR10v00_mass1[m]->Integral(binHigh1, binLow2);
        double signalEvents = dataSignal - estimatedBackground;
        
        // Print signal events
        std::cout << "Mass Bin: " << massCutLabels[m] << ", Working Point: " << wp_label << ", Number of signal events: " << signalEvents << std::endl;
        ...
        h_multijets_ljet_bJR10v00_mass1_scaled->SetFillColor(kBlue);
        h_multijets_ljet_bJR10v00_mass1_scaled->SetLineColor(kBlue);
        mc_stack->Add(h_multijets_ljet_bJR10v00_mass1_scaled);
        
        // Add signal to stack
        h_mc_ljet_bJR10v00_mass1[m]->SetFillColor(kRed);
        h_mc_ljet_bJR10v00_mass1[m]->SetLineColor(kRed);
        mc_stack->Add(h_mc_ljet_bJR10v00_mass1[m]);
        
        // Draw data points
        h_data_ljet_bJR10v00_mass1[m]->Draw("E");
        mc_stack->Draw("HIST SAME");
        h_data_ljet_bJR10v00_mass1[m]->Draw("E SAME");
        
        // Create legend
        TLegend* mass_legend = new TLegend(0.6, 0.7, 0.89, 0.89);
        mass_legend->SetBorderSize(0);
        mass_legend->AddEntry(h_data_ljet_bJR10v00_mass1[m], "Data", "lp");
        mass_legend->AddEntry(h_mc_ljet_bJR10v00_mass1[m], "Z (#rightarrow bb) + #gamma", "f");
        mass_legend->AddEntry(h_multijets_ljet_bJR10v00_mass1_scaled, "#gamma + jets", "f");
        mass_legend->Draw();
        ...
        if (m == 0) {
            ...
            mass_text.DrawLatex(0.2, 0.75, Form("Mass Bin: %d < m < %d GeV", (int)massCutLow[m], (int)massCutHigh[m]));
        }
        
        mass_canvas->SaveAs(Form("../Plots/DB_Deco/FitRdbMcut/%s/%s/jet_mass_comparison.pdf", wp_label, fileSystemMassCutLabels[m]));
        ...
        Legend1->AddEntry(h_mc_R_DB[m], "Z (#rightarrow bb) + #gamma", "f");
        ...
        mc_canvas->SaveAs(Form("../Plots/DB_Deco/FitRdbMcut/%s/%s/Fit_R_DB_mc.pdf", wp_label, fileSystemMassCutLabels[m]));
        
        // Save MC fit parameters
        SaveFitParameters(Form("../Plots/DB_Deco/FitRdbMcut/%s/%s/mc_fit_parameters.txt", wp_label, massCutLabels[m]),
                          Form("MC: Z (#rightarrow bb) + Jets (Mass Bin: %s, WP: %s)", massCutLabels[m], wp_label),
                          Mean_mc, meanErr_mc, 
        ...
        Legend_bkg->AddEntry(h_multijets_ljet_R_DB[m], "#gamma + jets", "f");
        ...
        bkg_canvas->SaveAs(Form("../Plots/DB_Deco/FitRdbMcut/%s/%s/Fit_R_DB_bkg.pdf", wp_label, fileSystemMassCutLabels[m]));
        
        // Save background fit parameters
        SaveFitParameters(Form("../Plots/DB_Deco/FitRdbMcut/%s/%s/bkg_fit_parameters.txt", wp_label, massCutLabels[m]),
                          Form("Background: Multijets (Mass Bin: %s, WP: %s)", massCutLabels[m], wp_label),
        ...
        data_canvas->SaveAs(Form("../Plots/DB_Deco/FitRdbMcut/%s/%s/Fit_R_DB_data.pdf", wp_label, fileSystemMassCutLabels[m]));
        
        // Save data fit parameters
        SaveFitParameters(Form("../Plots/DB_Deco/FitRdbMcut/%s/%s/data_fit_parameters.txt", wp_label, massCutLabels[m]),
                          Form("Data (Mass Bin: %s, WP: %s)", massCutLabels[m], wp_label),
        ...
        Legend_combined->AddEntry(h_combined_R_DB[m], "Combined MC", "f");
        ...
        combined_canvas->SaveAs(Form("../Plots/DB_Deco/FitRdbMcut/%s/%s/Fit_R_DB_combined.pdf", wp_label, fileSystemMassCutLabels[m]));
        
        // Save combined MC fit parameters
        SaveFitParameters(Form("../Plots/DB_Deco/FitRdbMcut/%s/%s/combined_fit_parameters.txt", wp_label, massCutLabels[m]),
                          Form("Combined MC (Mass Bin: %s, WP: %s)", massCutLabels[m], wp_label),
        ...
        TGraphAsymmErrors* err_band = new TGraphAsymmErrors(h_mc_total_for_error_band);//(h_mc_total_for_error_band->GetNbinsX()); // Initialize with number of bins
        ...
        err_band->SetFillStyle(3354);              // Hatch pattern
        ...
        TGraphAsymmErrors* err_band_ratio = new TGraphAsymmErrors(h_combined_clone_comb);//(h_combined_clone_comb->GetNbinsX());
        ...
        err_band_ratio->SetFillStyle(3354);  // Consistent style
        ...
        combinedstack_canvas->SaveAs(Form("../Plots/DB_Deco/FitRdbMcut/%s/%s/R_DB_stack_ratio_combined.pdf", wp_label, fileSystemMassCutLabels[m]));

        // Clean up individual canvas and objects inside the loop
        /*delete fit_canvas;
        delete ratio_graph;
        delete fit_func;
        delete h_multijets_ljet_bJR10v00_mass1_scaled; // Delete the cloned histogram
        delete mass_canvas;
        delete mc_stack;
        delete mass_legend;
        delete mc_canvas;
    ...
    DrawAtlasLabel(wp_label); // Pass working point label
    mean_canvas->SaveAs(Form("../Plots/DB_Deco/FitRdbMcut/%s/R_DB_mean_vs_masscut.pdf", wp_label));
    /*delete mean_canvas;
    delete g_mc_mean;
    delete g_data_mean;
    delete g_bkg_mean;
    ...
    DrawAtlasLabel(wp_label); // Pass working point label
    sigma_canvas->SaveAs(Form("../Plots/DB_Deco/FitRdbMcut/%s/R_DB_sigma_vs_masscut.pdf", wp_label));
    /*delete sigma_canvas;
    delete g_mc_sigma;
    delete g_data_sigma;
    delete g_bkg_sigma;
    ...
    // Find min/max for dynamic y-axis for mean ratios  --- Added 19 June 2025 ----
    double min_mean_ratio = std::numeric_limits<double>::max();
    double max_mean_ratio = std::numeric_limits<double>::min();

    for (int i = 0; i < nMassCuts; ++i) {
        if (mean_data_mc_ratio[i] < min_mean_ratio) min_mean_ratio = mean_data_mc_ratio[i];
        if (mean_data_mc_ratio[i] > max_mean_ratio) max_mean_ratio = mean_data_mc_ratio[i];

        if (mean_data_combined_ratio[i] < min_mean_ratio) min_mean_ratio = mean_data_combined_ratio[i];
        if (mean_data_combined_ratio[i] > max_mean_ratio) max_mean_ratio = mean_data_combined_ratio[i];

        if (mean_bkg_mc_ratio[i] < min_mean_ratio) min_mean_ratio = mean_bkg_mc_ratio[i];
        if (mean_bkg_mc_ratio[i] > max_mean_ratio) max_mean_ratio = mean_bkg_mc_ratio[i];
    }

    double y_mean_padding = (max_mean_ratio - min_mean_ratio) * 0.05; // 5% padding
    double y_mean_min = min_mean_ratio - y_mean_padding;
    double y_mean_max = max_mean_ratio + y_mean_padding; 

    // --- End Added 19 June 2025 ----
    ...
    g_mean_data_mc_ratio->GetYaxis()->SetRangeUser(y_mean_min, y_mean_max); // Dynamic range(0.95, 1.05);
    ...
    DrawAtlasLabel(wp_label); // Pass working point label
    mean_ratio_canvas->SaveAs(Form("../Plots/DB_Deco/FitRdbMcut/%s/R_DB_mean_ratio_vs_masscut.pdf", wp_label));
    ...
    // Find min/max for dynamic y-axis for sigma ratios
    double min_sigma_ratio = std::numeric_limits<double>::max();
    double max_sigma_ratio = std::numeric_limits<double>::min();

    for (int i = 0; i < nMassCuts; ++i) {
        if (sigma_data_mc_ratio[i] < min_sigma_ratio) min_sigma_ratio = sigma_data_mc_ratio[i];
        if (sigma_data_mc_ratio[i] > max_sigma_ratio) max_sigma_ratio = sigma_data_mc_ratio[i];

        if (sigma_data_combined_ratio[i] < min_sigma_ratio) min_sigma_ratio = sigma_data_combined_ratio[i];
        if (sigma_data_combined_ratio[i] > max_sigma_ratio) max_sigma_ratio = sigma_data_combined_ratio[i];

        if (sigma_bkg_mc_ratio[i] < min_sigma_ratio) min_sigma_ratio = sigma_bkg_mc_ratio[i];
        if (sigma_bkg_mc_ratio[i] > max_sigma_ratio) max_sigma_ratio = sigma_bkg_mc_ratio[i];
    }

    double y_sigma_padding = (max_sigma_ratio - min_sigma_ratio) * 0.05; // 5% padding
    double y_sigma_min = min_sigma_ratio - y_sigma_padding;
    double y_sigma_max = max_sigma_ratio + y_sigma_padding;
    ...
    g_sigma_data_mc_ratio->GetYaxis()->SetRangeUser(y_sigma_min, y_sigma_max); // Dynamic range (0.95, 1.05);
    ...
    DrawAtlasLabel(wp_label); // Pass working point label
    sigma_ratio_canvas->SaveAs(Form("../Plots/DB_Deco/FitRdbMcut/%s/R_DB_sigma_ratio_vs_masscut.pdf", wp_label));
    ...
    */
    ...
    ofstream summaryTable(Form("../Plots/DB_Deco/FitRdbMcut/%s/R_DB_summary_table.txt", wp_label));
    if (summaryTable.is_open()) {
        ...
        for (int m = 0; m < nMassCuts; m++) {
            summaryTable << "Mass Bin: " << massCutLabels[m] << endl;
            summaryTable << "-----------------------------" << endl;
        ...
        summaryTable.close();
        std::cout << "Summary table saved to ../Plots/DB_Deco/FitRdbMcut/" << wp_label << "/R_DB_summary_table.txt" << std::endl;
    } else {
    ...
    /*for (int m = 0; m < nMassCuts; m++) {
        delete h_mc_ljet_bJR10v00_mass1[m];
        delete h_data_ljet_bJR10v00_mass1[m];
        delete h_multijets_ljet_bJR10v00_mass1[m];
        delete h_mc_R_DB[m];
