...
void CreateDirectories(const char* wp_label) {
    ...
    TString baseDirCmd = Form("mkdir -p ../Plots/DB/FitRdbMcut/%s", wp_label);
    ...
    for (int i = 0; i < nMassCuts; i++) {
        TString dirCmd = Form("mkdir -p ../Plots/DB/FitRdbMcut/%s/%s", wp_label, fileSystemMassCutLabels[i]);
...
void RunRdbAnalysis(const char* wp_label) {
    ...
    //-------------------------------------------
    TString mcFileName = Form("../../NtupleSlim/DB_0p%swp_slim/Zbby_0p%swp_slim.root", wp_label, wp_label);
    TString dataFileName = Form("../../NtupleSlim/DB_0p%swp_slim/data_0p%swp_slim.root", wp_label, wp_label);
    TString mulijetsFileName = Form("../../NtupleSlim/DB_0p%swp_slim/gamma_jets_0p%swp_slim.root", wp_label, wp_label);
    ...
    for (int m = 0; m < nMassCuts; m++) {
        ...
        fit_canvas->SaveAs(Form("../Plots/DB/FitRdbMcut/%s/%s/scale_factor_fit.pdf", wp_label, fileSystemMassCutLabels[m]));
        ...
        mass_legend->AddEntry(h_mc_ljet_m1[m], "Z (#rightarrow bb) + #gamma", "f");
        mass_legend->AddEntry(h_multijets_ljet_m1_scaled, "#gamma + jets", "f");
        mass_legend->Draw();
        ...
        mass_canvas->SaveAs(Form("../Plots/DB/FitRdbMcut/%s/%s/jet_mass_comparison.pdf", wp_label, fileSystemMassCutLabels[m]));
        ...
        mc_canvas->SaveAs(Form("../Plots/DB/FitRdbMcut/%s/%s/Fit_R_DB_mc.pdf", wp_label, fileSystemMassCutLabels[m]));
        
        // Save MC fit parameters
        SaveFitParameters(Form("../Plots/DB/FitRdbMcut/%s/%s/mc_fit_parameters.txt", wp_label, massCutLabels[m]),
        ...
        bkg_canvas->SaveAs(Form("../Plots/DB/FitRdbMcut/%s/%s/Fit_R_DB_bkg.pdf", wp_label, fileSystemMassCutLabels[m]));
        
        // Save background fit parameters
        SaveFitParameters(Form("../Plots/DB/FitRdbMcut/%s/%s/bkg_fit_parameters.txt", wp_label, massCutLabels[m]),
        ...
        data_canvas->SaveAs(Form("../Plots/DB/FitRdbMcut/%s/%s/Fit_R_DB_data.pdf", wp_label, fileSystemMassCutLabels[m]));
        
        // Save data fit parameters
        SaveFitParameters(Form("../Plots/DB/FitRdbMcut/%s/%s/data_fit_parameters.txt", wp_label, massCutLabels[m]),
        ...
        combined_canvas->SaveAs(Form("../Plots/DB/FitRdbMcut/%s/%s/Fit_R_DB_combined.pdf", wp_label, fileSystemMassCutLabels[m]));
        
        // Save combined MC fit parameters
        SaveFitParameters(Form("../Plots/DB/FitRdbMcut/%s/%s/combined_fit_parameters.txt", wp_label, massCutLabels[m]),
        ...
        TGraphAsymmErrors* err_band_ratio = new TGraphAsymmErrors(h_combined_clone_comb);//(h_mc_total_for_error_band->GetNbinsX());
        ...
        combinedstack_canvas->SaveAs(Form("../Plots/DB/FitRdbMcut/%s/%s/R_DB_stack_ratio_combined.pdf", wp_label, fileSystemMassCutLabels[m]));
    ...
    DrawAtlasLabel(wp_label); // Pass working point label
    mean_canvas->SaveAs(Form("../Plots/DB/FitRdbMcut/%s/R_DB_mean_vs_masscut.pdf", wp_label));
    /*delete mean_canvas;
    delete g_mc_mean;
    delete g_data_mean;
    delete g_bkg_mean;
    ...
    DrawAtlasLabel(wp_label); // Pass working point label
    sigma_canvas->SaveAs(Form("../Plots/DB/FitRdbMcut/%s/R_DB_sigma_vs_masscut.pdf", wp_label));
    /*delete sigma_canvas;
    delete g_mc_sigma;
    delete g_data_sigma;
    delete g_bkg_sigma;
    ...
    DrawAtlasLabel(wp_label); // Pass working point label
    mean_ratio_canvas->SaveAs(Form("../Plots/DB/FitRdbMcut/%s/R_DB_mean_ratio_vs_masscut.pdf", wp_label));
    ...
    DrawAtlasLabel(wp_label); // Pass working point label
    sigma_ratio_canvas->SaveAs(Form("../Plots/DB/FitRdbMcut/%s/R_DB_sigma_ratio_vs_masscut.pdf", wp_label));
    ...
    */
    ...
    ofstream summaryTable(Form("../Plots/DB/FitRdbMcut/%s/R_DB_summary_table.txt", wp_label));
    if (summaryTable.is_open()) {
        ...
        summaryTable.close();
        std::cout << "Summary table saved to ../Plots/DB/FitRdbMcut/" << wp_label << "/R_DB_summary_table.txt" << std::endl;
    } else {
