...
void MassIsolationExtractSig_MJB(const std::string& wp) {
    ...
    std::string outputDirPath = "../Plots/DB_Deco/MassIsolation/" + fileWP + "/";
    gSystem->Exec(("mkdir -p " + outputDirPath).c_str());
    
    // Construct input file paths dynamically
    std::string mcPath = "../../NtupleSlim/DB_Deco_" + fileWP + "wp_slim/Zbby_deco_" + fileWP + "wp_slim.root";
    std::string dataPath = "../../NtupleSlim/DB_Deco_" + fileWP + "wp_slim/data_deco_" + fileWP + "wp_slim.root";
    std::string multijetsPath = "../../NtupleSlim/DB_Deco_" + fileWP + "wp_slim/gamma_jets_deco_" + fileWP + "wp_slim.root";
    ...
    TH1F *h_mc_ljet_bJR10v00_mass = new TH1F("h_mc_ljet_bJR10v00_mass", "Large-R Jet Mass", 20, 50, 160);
    TH1F *h_data_ljet_bJR10v00_mass = new TH1F("h_data_ljet_bJR10v00_mass", "Large-R Jet Mass", 20, 50, 160);
    TH1F *h_multijets_ljet_bJR10v00_mass = new TH1F("h_multijets_ljet_bJR10v00_mass", "Large-R Jet Mass", 20, 50, 160);

    // Enable Sumw2 for data histogram to ensure proper error calculation
    //h_data_ljet_bJR10v00_mass->Sumw2();
    // Enable Sumw2 for MC histogram to ensure proper error calculation for MC signal band
    //h_mc_ljet_bJR10v00_mass->Sumw2();

    // Set histogram styling
    h_data_ljet_bJR10v00_mass->SetMarkerStyle(20);
    h_data_ljet_bJR10v00_mass->SetMarkerSize(0.8);
    h_data_ljet_bJR10v00_mass->SetMarkerColor(kBlack);
    h_data_ljet_bJR10v00_mass->SetLineColor(kBlack);

    h_multijets_ljet_bJR10v00_mass->SetFillColor(kBlue);
    h_multijets_ljet_bJR10v00_mass->SetLineColor(kBlue);

    h_mc_ljet_bJR10v00_mass->SetFillColor(kRed);
    h_mc_ljet_bJR10v00_mass->SetLineColor(kRed);
    ...
    tree_multijets->SetBranchAddress("ljet_bJR10v00_mass", &ljet_bJR10v00_mass);
    ...
    for (Long64_t i = 0; i < nentries; i++) {
        tree_mc->GetEntry(i);
        h_mc_ljet_bJR10v00_mass->Fill((*ljet_bJR10v00_mass)[0]/1000.0, weight); // Convert MeV to GeV
    }
    ...
    for (Long64_t i = 0; i < nentries; i++) {
        ...
        h_data_ljet_bJR10v00_mass->Fill((*ljet_bJR10v00_mass)[0]/1000.0); // Convert MeV to GeV
    }
    ...
    for (Long64_t i = 0; i < nentries; i++) {
        ...
        h_multijets_ljet_bJR10v00_mass->Fill((*ljet_bJR10v00_mass)[0]/1000.0, weight_multijets); // Convert MeV to GeV
    }

    // Define sideband regions
    int binLow1 = h_multijets_ljet_bJR10v00_mass->FindBin(50);
    int binHigh1 = h_multijets_ljet_bJR10v00_mass->FindBin(70);
    int binLow2 = h_multijets_ljet_bJR10v00_mass->FindBin(110);
    int binHigh2 = h_multijets_ljet_bJR10v00_mass->FindBin(160);
    ...
    for (int bin = binLow1; bin <= binHigh1; ++bin) {
        double bin_center = h_multijets_ljet_bJR10v00_mass->GetBinCenter(bin);
        double data_content = h_data_ljet_bJR10v00_mass->GetBinContent(bin);
        double mc_content = h_multijets_ljet_bJR10v00_mass->GetBinContent(bin);
        ...
        if (mc_content > 0) {
            ...
            double data_error = h_data_ljet_bJR10v00_mass->GetBinError(bin);
            double mc_error = h_multijets_ljet_bJR10v00_mass->GetBinError(bin);
    ...
    TF1* fit_func = new TF1("fit_func", "[0] + [1] * x", 50, 160);
    ...
    TH1F* h_multijets_ljet_bJR10v00_mass_scaled = (TH1F*)h_multijets_ljet_bJR10v00_mass->Clone("h_multijets_ljet_bJR10v00_mass_scaled");
    
    // Apply mass-dependent scale factor bin by bin
    for (int bin = 1; bin <= h_multijets_ljet_bJR10v00_mass_scaled->GetNbinsX(); ++bin) {
        double bin_center = h_multijets_ljet_bJR10v00_mass_scaled->GetBinCenter(bin);
        double content = h_multijets_ljet_bJR10v00_mass_scaled->GetBinContent(bin);
        double error = h_multijets_ljet_bJR10v00_mass_scaled->GetBinError(bin);
        
        // Calculate bin-by-bin scale factor
        double scale_factor = p0 + p1 * bin_center;
        
        // Apply the scale factor
        h_multijets_ljet_bJR10v00_mass_scaled->SetBinContent(bin, content * scale_factor);
        h_multijets_ljet_bJR10v00_mass_scaled->SetBinError(bin, error * scale_factor);
    }

    // Estimate background in the signal region using scaled histogram
    double estimatedBackground = h_multijets_ljet_bJR10v00_mass_scaled->Integral(binHigh1+1, binLow2-1);
    
    // Calculate signal events
    double dataSignal = h_data_ljet_bJR10v00_mass->Integral(binHigh1+1, binLow2-1);
    ...
    TF1 *polyExpFit = new TF1("polyExpFit", "[0]*exp([1]*x + [2]*x*x + [3]*x*x*x)", 50, 160);
    
    // Clone the data histogram for fitting the sidebands
    TH1F *h_data_sidebands = (TH1F*)h_data_ljet_bJR10v00_mass->Clone("h_data_sidebands");
    ...
    TH1F *h_signal_ljet_bJR10v00_mass = (TH1F*)h_data_ljet_bJR10v00_mass->Clone("h_signal_ljet_bJR10v00_mass");
    h_signal_ljet_bJR10v00_mass->SetTitle("Isolated Signal (Data);M^{J} [GeV];Events"); // Set title for drawing
    
    // Subtract the fit from the data to isolate the signal
    
    for (int i = 1; i <= h_signal_ljet_bJR10v00_mass->GetNbinsX(); ++i) {
        double binCenter = h_signal_ljet_bJR10v00_mass->GetBinCenter(i);
        double binContent = h_signal_ljet_bJR10v00_mass->GetBinContent(i);
        double fitValue = polyExpFit->Eval(binCenter);
        h_signal_ljet_bJR10v00_mass->SetBinContent(i, binContent - fitValue);
        
        // Set negative values to zero
        if (h_signal_ljet_bJR10v00_mass->GetBinContent(i) < 0) {
            h_signal_ljet_bJR10v00_mass->SetBinContent(i, 0);
        }
    }
    ...
    TH1F *h_background_fit = (TH1F*)h_data_ljet_bJR10v00_mass->Clone("h_background_fit_for_subtraction");
    ...
    h_signal_ljet_bJR10v00_mass->Add(h_background_fit, -1.0);

    // Set negative values to zero after subtraction (and adjust errors accordingly)
    for (int i = 1; i <= h_signal_ljet_bJR10v00_mass->GetNbinsX(); ++i) {
        if (h_signal_ljet_bJR10v00_mass->GetBinContent(i) < 0) {
            h_signal_ljet_bJR10v00_mass->SetBinContent(i, 0);
            h_signal_ljet_bJR10v00_mass->SetBinError(i, 0); // Error also becomes 0 if content is 0
        }
    }
    ...
    hs->Add(h_multijets_ljet_bJR10v00_mass_scaled);
    hs->Add(h_mc_ljet_bJR10v00_mass);
    ...
    h_data_ljet_bJR10v00_mass->Draw("EP same");


    hs->GetXaxis()->SetLabelSize(0);
    h_data_ljet_bJR10v00_mass->GetXaxis()->SetLabelSize(0);
    ...
    legend->AddEntry(h_data_ljet_bJR10v00_mass, "Data", "lep");
    legend->AddEntry(h_mc_ljet_bJR10v00_mass, "Z (#rightarrow bb) + #gamma", "f");
    legend->AddEntry(h_multijets_ljet_bJR10v00_mass_scaled, "#gamma + jets", "f");
    ...
    TH1F* h_ratio = (TH1F*)h_data_ljet_bJR10v00_mass->Clone("h_ratio");
    ...
    h_data_ljet_bJR10v00_mass->Draw("EP");
    polyExpFit->Draw("same");
    
    // Highlight sideband regions
    double xlow1 = h_data_ljet_bJR10v00_mass->GetBinLowEdge(binLow1);
    double xhigh1 = h_data_ljet_bJR10v00_mass->GetBinLowEdge(binHigh1+1);
    double xlow2 = h_data_ljet_bJR10v00_mass->GetBinLowEdge(binLow2);
    double xhigh2 = h_data_ljet_bJR10v00_mass->GetBinLowEdge(binHigh2+1);
    
    TBox *sidebandBox1 = new TBox(xlow1, 0, xhigh1, h_data_ljet_bJR10v00_mass->GetMaximum()*1.75);
    sidebandBox1->SetFillColorAlpha(kYellow, 0.3);
    sidebandBox1->Draw("same");
    
    TBox *sidebandBox2 = new TBox(xlow2, 0, xhigh2, h_data_ljet_bJR10v00_mass->GetMaximum()*1.75);
    ...
    legendSideband->AddEntry(h_data_ljet_bJR10v00_mass, "Data", "lep");
    ...
    h_mc_ljet_bJR10v00_mass->SetFillColor(kRed);
    h_mc_ljet_bJR10v00_mass->GetYaxis()->SetRangeUser(0, h_mc_ljet_bJR10v00_mass->GetMaximum()*1.9);
    h_mc_ljet_bJR10v00_mass->GetXaxis()->SetTitle("M^{J} [GeV]");
    h_mc_ljet_bJR10v00_mass->GetYaxis()->SetTitle("Events");
    h_mc_ljet_bJR10v00_mass->Draw("HIST");

    // Create and draw the statistical uncertainty band for the MC signal
    TGraphAsymmErrors* mc_signal_err_band = new TGraphAsymmErrors(h_mc_ljet_bJR10v00_mass);
    for (int i = 0; i < mc_signal_err_band->GetN(); ++i) {
        double content = h_mc_ljet_bJR10v00_mass->GetBinContent(i + 1);
        double error = h_mc_ljet_bJR10v00_mass->GetBinError(i + 1);
    ...
    */
    ...
    h_signal_ljet_bJR10v00_mass->SetMarkerStyle(20);
    h_signal_ljet_bJR10v00_mass->SetMarkerColor(kBlack);
    h_signal_ljet_bJR10v00_mass->Draw("EP same");
    
    // Create and draw the statistical uncertainty band for the isolated data signal
    TGraphAsymmErrors* signal_err_band = new TGraphAsymmErrors(h_signal_ljet_bJR10v00_mass);
    for (int i = 0; i < signal_err_band->GetN(); ++i) {
        double content = h_signal_ljet_bJR10v00_mass->GetBinContent(i + 1);
        double error = h_signal_ljet_bJR10v00_mass->GetBinError(i + 1);
    ...
    */
    ...
    TF1 *mc_gauss_fit = new TF1("mc_gauss_fit", "gaus", 85, 110); //85, 110
    
    // Set initial parameters for MC Gaussian fit
    mc_gauss_fit->SetParameter(0, h_mc_ljet_bJR10v00_mass->GetMaximum());  // Amplitude
    ...
    mc_gauss_fit->SetParameter(1, h_mc_ljet_bJR10v00_mass->GetXaxis()->GetBinCenter(h_mc_ljet_bJR10v00_mass->GetMaximumBin()));  // Mean
    mc_gauss_fit->SetParameter(2, h_mc_ljet_bJR10v00_mass->GetRMS() * 0.7); // Sigma (scaled RMS as a rough estimate)
    
    h_mc_ljet_bJR10v00_mass->Fit(mc_gauss_fit, "R");
    ...
    TF1 *data_gauss_fit = new TF1("data_gauss_fit", "gaus", 85, 110); //85, 110
    
    // Set initial parameters for Data Gaussian fit
    data_gauss_fit->SetParameter(0, h_signal_ljet_bJR10v00_mass->GetMaximum());  // Amplitude
    //data_gauss_fit->SetParameter(1, 90);  // Mean
    //data_gauss_fit->SetParameter(2, 10);  // Sigma

    // Use the histogram's properties to get more robust initial guesses
    data_gauss_fit->SetParameter(1, h_signal_ljet_bJR10v00_mass->GetXaxis()->GetBinCenter(h_signal_ljet_bJR10v00_mass->GetMaximumBin()));  // Mean
    data_gauss_fit->SetParameter(2, h_signal_ljet_bJR10v00_mass->GetRMS() * 0.7); // Sigma (scaled RMS as a rough estimate)
    
    h_signal_ljet_bJR10v00_mass->Fit(data_gauss_fit, "R");
    ...
    // Create a legend
    TLegend *legendSignal = new TLegend(0.45, 0.7, 0.9, 0.95); 
    legendSignal->SetBorderSize(0);
    legendSignal->SetTextSize(0.018); 
    legendSignal->AddEntry(h_signal_ljet_bJR10v00_mass, "Isolated Signal (Data)", "lep");
    //legendSignal->AddEntry(signal_err_band, "Isolated Signal (Stat. Unc.)", "f"); // Added entry for data error band
    legendSignal->AddEntry(h_mc_ljet_bJR10v00_mass, "MC Signal", "f");
    ...
    if (summary.is_open()) {
        ...
        summary << "----------------" << endl;
        summary << "Signal Region: " << h_data_ljet_bJR10v00_mass->GetBinLowEdge(binHigh1+1) << " - " << h_data_ljet_bJR10v00_mass->GetBinLowEdge(binLow2) << " GeV" << endl;
